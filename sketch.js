...
function setup() {
  ...
  back=createSprite(width/2+300,height/2+30,width,20);//height/2+30 is given to crop the bottom side of background image
...
function draw() {
  ...
  if (gameState === PLAY) {
    ...
    //camera moves forward with trex
    ...
    //if (camera.position.x> ground.x) {//trex will be moving but seems to stop at centre
    ...
    if ((touches.length>0 || keyDown("space")) && trex.y>= windowHeight-80){
      ...
      if(soundFlag ==="silent"){//play sound once when space key is pressed and change sound flag value to played
        jumpSound.play();
        soundFlag = "played";
      }
...
function spawnObstacles() {
  if (frameCount % 120 === 0) {
    ...
    obstacle.scale = 0.5;
    obstacle.lifetime =200;// abs(width/obstacle.velocityX);
...
function spawnClouds() {
  ...
  if (frameCount % 100 === 0) {
    ...
    cloud.lifetime = 200;//abs(width/cloud.velocityX);
